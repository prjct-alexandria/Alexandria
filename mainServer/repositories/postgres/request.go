...
func (r PgRequestRepository) CreateRequest(req entities.Request) (entities.Request, error) {
	...
	stmt, err := r.Db.Prepare("INSERT INTO request (articleID, sourceVersionID, sourceHistoryID, targetVersionID, targetHistoryID, conflicted) VALUES ($1,$2,$3,$4,$5,$6) RETURNING id,state")
	if err != nil {
...
func (r PgRequestRepository) UpdateRequest(req entities.Request) error {
	...
	stmt, err := r.Db.Prepare("UPDATE request SET articleID=$2, sourceVersionID=$3, sourceHistoryID=$4, targetVersionID=$5, targetHistoryID=$6, conflicted=$7 WHERE id=$1")
	if err != nil {
		return err
	}

	// TODO: check if rows were affected, and return error if not
	_, err = stmt.Exec(req.RequestID, req.ArticleID, req.SourceVersionID, req.SourceHistoryID, req.TargetVersionID, req.TargetHistoryID, req.Conflicted)
	if err != nil {
		return err
	}
	return nil
...
func (r PgRequestRepository) GetRequest(request int64) (entities.Request, error) {
	...
	err = row.Scan(&req.RequestID, &req.ArticleID, &req.SourceVersionID, &req.SourceHistoryID, &req.TargetVersionID, &req.TargetHistoryID, &req.State, &req.Conflicted)
	if err != nil {
		return entities.Request{}, err
	}
