...
func (repo FilesystemRepository) AddFilesInDirToZip(zipWriter *zip.Writer, dirPath string, dirInZip string) error {
	...
	for _, file := range files {
		...
		err := repo.addFileInDirToZip(file, zipWriter, dirPath, dirInZip)
		if err != nil {
			return err
		}
	}
...
func (repo FilesystemRepository) addFileInDirToZip(file fs.FileInfo, zipWriter *zip.Writer, dirPath string, dirInZip string) error {
	if file.IsDir() {
